...
on disk, in the background after a session starts. See
[constraints C20](constraints.md) for what that key scopes to and why it is
written.

A team can opt out with `--no-auto-update`, and you cannot see which ones did.
The asymmetry only runs one way — **assume none of them opted out.**
...
auto-updating machine at once, on its next session, and neither outcome is
