<!-- GENERATED from plugins/himoa/standards/resumption.md by tests/validate-adapter-projection.mjs (himoa 3.3.0). DO NOT EDIT. Edit the canonical source and run: node tests/validate-adapter-projection.mjs --write -->
...
| Holding the run's position when the conversation is summarised away | The framework's, and already solved | The run state file in §2, plus the ledger `@HIMOA_HOME@/standards/gate-handoff.md` §5 owns |
...
`@HIMOA_HOME@/standards/gate-handoff.md` §5, which owns that question;
...
`@HIMOA_HOME@/standards/untrusted-content.md` governs it in full.
...
balanced, escalate** —
`@HIMOA_HOME@/standards/execution-efficiency.md` §5, whose rule that
