...
<!-- GENERATED from plugins/himoa/skills/gate-approve/SKILL.md by tests/validate-adapter-projection.mjs (himoa 3.3.0). DO NOT EDIT. Edit the canonical source and run: node tests/validate-adapter-projection.mjs --write -->
...
`@HIMOA_HOME@/standards/resumption.md` §5 and §6 own it. Compare what
...
- **for a defect, the root cause and its label** — demonstrated, or `UNKNOWN`
  ...
  `himoa-domain-debugging` owns what
  demonstrated means;
...
`@HIMOA_HOME@/standards/resumption.md` §3 governs. Under `work-item`
