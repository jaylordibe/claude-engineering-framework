...
<!-- GENERATED from plugins/himoa/skills/gate-validate/SKILL.md by tests/validate-adapter-projection.mjs (himoa 3.4.0). DO NOT EDIT. Edit the canonical source and run: node tests/validate-adapter-projection.mjs --write -->
...
`@HIMOA_HOME@/standards/evidence.md`. Read it first.
...
`@HIMOA_HOME@/standards/evidence.md` §1:
...
- a quality threshold lowered, a rule or check removed, or an assertion
  ...
  simplification (`@HIMOA_HOME@/standards/evidence.md` §5);
...
`@HIMOA_HOME@/standards/evidence.md` §7. Where a result genuinely still
...
`@HIMOA_HOME@/standards/execution-efficiency.md` §10, which this does
...
`@HIMOA_HOME@/standards/evidence.md` §6 requires those rows and §9
above requires the verdict they produce. What it removes is log volume that
proves nothing, and only that.

## 10. Evidence report

Use `@HIMOA_HOME@/templates/validation-report.md`. Include a row for
