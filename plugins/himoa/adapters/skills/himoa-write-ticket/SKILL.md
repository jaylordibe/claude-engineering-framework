...
<!-- GENERATED from plugins/himoa/skills/write-ticket/SKILL.md by tests/validate-adapter-projection.mjs (himoa 3.4.1). DO NOT EDIT. Edit the canonical source and run: node tests/validate-adapter-projection.mjs --write -->
...
`@HIMOA_HOME@/standards/execution-efficiency.md` says of every stage.
...
`@HIMOA_HOME@/standards/repository-evidence.md` §5 then grades that
...
- **Never fill a gap with something plausible.** A fact you did not verify is
  ...
  the repository answers it — `@HIMOA_HOME@/standards/repository-evidence.md`
...
2. **A goal, a request, rough notes or a pasted conversation** — the common
   ...
   `@HIMOA_HOME@/standards/untrusted-content.md` gives the word: it
...
ticket in the shape of `@HIMOA_HOME@/templates/ticket.md`, with every
gap labelled.

Before drafting, read the repository for the area the goal names. This is a
**bounded, targeted read** in the sense of
`@HIMOA_HOME@/standards/execution-efficiency.md` §3 — the entry
...
Follow `@HIMOA_HOME@/standards/execution-efficiency.md`. Use the
...
`himoa-domain-debugging` downstream.
...
`@HIMOA_HOME@/standards/repository-evidence.md` §5 owns. The outcome
...
The shape is `@HIMOA_HOME@/templates/ticket.md`. What each section
...
`himoa-domain-debugging` owns the order in
...
3. **Re-emit the whole substantive ticket.** Never a diff, never "updated the
   ...
   (`@HIMOA_HOME@/standards/execution-efficiency.md` §9).
...
- **Scope that only looks like scope.** A constraint on how the outcome is
  ...
  new feature. `@HIMOA_HOME@/standards/repository-evidence.md` §4c
...
restated — `@HIMOA_HOME@/standards/execution-efficiency.md` §11. A
...
- **The narrow read that should have widened.** A tidy, confident, short draft
  ...
  `@HIMOA_HOME@/standards/execution-efficiency.md` §3 calls the most
