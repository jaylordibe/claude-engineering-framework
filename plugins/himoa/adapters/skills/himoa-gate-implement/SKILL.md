...
<!-- GENERATED from plugins/himoa/skills/gate-implement/SKILL.md by tests/validate-adapter-projection.mjs (himoa 3.7.0). DO NOT EDIT. Edit the canonical source and run: node tests/validate-adapter-projection.mjs --write -->
...
- under `work-item`, the approval stage writes the approved scope, the risk
  ...
  `@HIMOA_HOME@/standards/resumption.md` §3 governs. **Read it before
  ...
  it. `@HIMOA_HOME@/standards/gate-handoff.md` §5 owns that question
...
`@HIMOA_HOME@/standards/resumption.md` §6 and honour its outcome:
...
`@HIMOA_HOME@/standards/untrusted-content.md` §3.1.
...
3. implement the smallest **coherent and complete** change, per
   `@HIMOA_HOME@/standards/architecture.md` §3;
...
where it is paid.** `@HIMOA_HOME@/standards/architecture.md` §3 owns
...
`@HIMOA_HOME@/standards/execution-efficiency.md` §2.1. A slice that was
...
`@HIMOA_HOME@/standards/`. This skill does not restate them.**
...
`@HIMOA_HOME@/standards/evidence.md` §4.
...
`@HIMOA_HOME@/standards/evidence.md` §7: a result that predates the
