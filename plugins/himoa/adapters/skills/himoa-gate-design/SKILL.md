...
<!-- GENERATED from plugins/himoa/skills/gate-design/SKILL.md by tests/validate-adapter-projection.mjs (himoa 3.4.1). DO NOT EDIT. Edit the canonical source and run: node tests/validate-adapter-projection.mjs --write -->
...
`@HIMOA_HOME@/standards/repository-evidence.md` §5 owns that split and
...
`@HIMOA_HOME@/standards/execution-efficiency.md` §3. Standard is the
...
Brief each on `@HIMOA_HOME@/standards/execution-efficiency.md` §8.5,
...
`@HIMOA_HOME@/standards/execution-efficiency.md` §8.4 owns it —
...
The governing rule is `@HIMOA_HOME@/standards/repository-evidence.md`.
...
`himoa-domain-debugging` owns the diagnosis
...
add. `@HIMOA_HOME@/standards/repository-evidence.md` §4c owns that
...
`Decision` list in `@HIMOA_HOME@/templates/plan.md` §5: every structure
...
`@HIMOA_HOME@/templates/threat-model.md`, in proportion to the tier.
...
repository's own documentation plus `@HIMOA_HOME@/standards/`. A plan
...
`@HIMOA_HOME@/templates/contract-change.md` when the change adds or
alters an observable surface, and
`@HIMOA_HOME@/templates/data-design.md` when it adds or alters a
persisted shape.
...
is `@HIMOA_HOME@/standards/testing.md` §3; the design work is deciding
...
Structure it on `@HIMOA_HOME@/templates/plan.md`, at the depth §4
...
Follow `@HIMOA_HOME@/standards/gate-handoff.md`, starting with its §0
