...
<!-- GENERATED from plugins/himoa/skills/gate-review/SKILL.md by tests/validate-adapter-projection.mjs (himoa 3.7.0). DO NOT EDIT. Edit the canonical source and run: node tests/validate-adapter-projection.mjs --write -->
...
`@HIMOA_HOME@/standards/execution-efficiency.md`; it does not change
anything this section requires.

Launch the selected agents **in parallel**, each in a fresh context, on the
brief `@HIMOA_HOME@/standards/execution-efficiency.md` §8.5 defines.
...
`@HIMOA_HOME@/templates/review-handoff.md`, which is where that list
lives.
...
`@HIMOA_HOME@/standards/architecture.md` §3 owns the catalogue; this
...
- **Correctness and security first, always.** Run the simplification pass
  **only after** the correctness and security findings above are resolved. A
  ...
  `@HIMOA_HOME@/standards/execution-efficiency.md` §1 is never traded
...
documentation first, then `@HIMOA_HOME@/standards/`. This skill does
...
`@HIMOA_HOME@/templates/review-handoff.md`, which is also the report
...
and a `path:line` for every claim — `@HIMOA_HOME@/standards/finding-report.md`
...
result. `@HIMOA_HOME@/standards/execution-efficiency.md` §8.4 owns what
...
`@HIMOA_HOME@/standards/execution-efficiency.md` §5 — a second
...
Use `@HIMOA_HOME@/templates/review-handoff.md`:
