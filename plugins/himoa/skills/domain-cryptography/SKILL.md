...
when_to_use: Use when a change touches a security-sensitive primitive — encryption or decryption, password or credential hashing, a message authentication code or signature, token or key or nonce generation, random values used for security, key derivation or storage, certificate or signature verification, or a security-token or webhook-signature format, and tests for any of those.
...
(`${CLAUDE_PLUGIN_ROOT}/standards/security.md`).
